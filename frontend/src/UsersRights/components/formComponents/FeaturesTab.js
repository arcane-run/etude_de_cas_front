...
class FeaturesTab extends Component {
  ...
  handleChangeRole = ({ target: { value } })  => {
    ...
    const outputSetting = update(this.props.editedSetting, { $set: editedSetting });
  ...
  handleChangeCheckBox = (e) => {
    ...
    if (isChecked) {
      cloneEditedSettings.claims.features_rights[feature] = rights_mapper[rights]
    } else {
      delete cloneEditedSettings.claims.features_rights[feature]
    }

    const outputSetting = update(this.props.editedSetting, { $set: cloneEditedSettings });
  ...
  render () {
    return (
        <div style={{ display: 'flex', flexDirection: 'row', height: '32%', justifyContent: 'space-between' }}>
            <CustomSelect
                id={'role'}
                value={this.props.computeRole(this.props.editedSetting.claims?.features_rights, this.props.roles)}
            ...
            <div>
                <Paper elevation={0} square={true} style={{ height: 300, width: '100%', overflow: 'auto' }}>
                    <Table>
                        ...
                        <TableBody >
                            {this.props.features.map(feature => (
                                <TableRow key={feature}>
                                    <TableCell align='right'>
                                        <Typography style={{ fontSize: 12 }}>
                                            {capitalize(feature).replace(/_/g, ' ')}
                                        </Typography>
                                    </TableCell>
                                    <TableCell padding='checkbox'>
                                        <Checkbox
                                            name={`${feature}-Admin`}
                                            checked={this.handleCheckbox(feature, 3)}
                                            onChange={this.handleChangeCheckBox}
                                            disabled={this.props.computeRole(this.props.editedSetting.features_rights, this.props.roles) === 'Custom' ||
                                              this.props.computeRole(this.props.editedSetting.features_rights, this.props.roles) === 'SmartFeeds User' ? false : true}/>
                                    </TableCell>
                                    <TableCell padding='checkbox'>
                                        <Checkbox
                                            name={`${feature}-Editor`}
                                            checked={this.handleCheckbox(feature, 2)}
                                            onChange={this.handleChangeCheckBox}
                                            disabled={this.props.computeRole(this.props.editedSetting.features_rights, this.props.roles) === 'Custom' ||
                                            this.props.computeRole(this.props.editedSetting.features_rights, this.props.roles) === 'SmartFeeds User' ? false : true}/>
                                    </TableCell>
                                    <TableCell padding='checkbox'>
                                        <Checkbox
                                            name={`${feature}-Viewer`}
                                            checked={this.handleCheckbox(feature, 1)}
                                            onChange={this.handleChangeCheckBox}
                                            disabled={this.props.computeRole(this.props.editedSetting.features_rights, this.props.roles) === 'Custom' ||
                                            this.props.computeRole(this.props.editedSetting.features_rights, this.props.roles) === 'SmartFeeds User' ? false : true}/>
                                    </TableCell>
