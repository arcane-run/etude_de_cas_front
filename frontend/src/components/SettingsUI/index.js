...
class SettingsUI extends React.Component {
    ...
    componentDidMount = async () => {
        ...
        if (this.props.match.params.id === 'new') {
            this.setState({ isSettingDialogOpen: true, isButtonSaveEnabled: true })
        } else if (this.props.match.params.id) {
            let settingFromUrl = this.state.settingsList.find(setting => setting[this.props.keyIndexName] === parseInt(this.props.match.params.id, 10))
            if (settingFromUrl !== undefined) {
                this.setState({ editedSetting: settingFromUrl, isSettingDialogOpen: true })
            } else {
                let basePath = this.props.match.url.replace(`/${this.props.match.params.id}`, '')
                let path = `${basePath}${getCommonSearchParamFromURL(this.props.location.search)}`
    ...
    getSetting = async () => {
        try {
            ...
            const data = await this.props.apiGetSettings();
            this.setState({ settingsList: data.users, isLoading: false });
        }
        catch (err) {
            this.setState({ errorDialog: true, isLoading: false, errorDetails: err.toString() })
        }
    ...
    updateSetting = async () => {
        try {
            ...
            const data = await this.props.apiPutSettings(editedSetting);

            let newSettings = this.state.settingsList;
            let idx = newSettings.findIndex((r) => {
                return r[this.props.keyIndexName] === data[this.props.keyIndexName]

            });
            newSettings = update(newSettings, { [idx]: { $set: data } });
            this.setState({
    ...
    postSetting = async () => {
        try {
            ...
            const data = await this.props.apiPostSettings(createdSetting);
            let newSettings = this.state.settingsList;
            newSettings = update(newSettings, { $push: [data] });
            let basePath = this.props.match.url.replace(this.props.match.params.id, data[this.props.keyIndexName])
            let path = `${basePath}${getCommonSearchParamFromURL(this.props.location.search)}`
            this.props.history.push(path)
    ...
    handleApiCall = async (apiCall) => {
        ...
        let idx = this.state.settingsList.findIndex(r => r[this.props.keyIndexName] === data[this.props.keyIndexName]);

        const newSettings = update(this.state.settingsList, { [idx]: { $set: data } });
    ...
    deleteSetting = async () => {
        try {
            ...
            let newSettings = settingsList.filter(setting => {
                return setting[this.props.keyIndexName] !== deletedSetting[this.props.keyIndexName]
            });
    ...
    openEditSettingDialog = (settingId, columnId) => {
        if (!this.props.ignoredColumns.includes(columnId)) {
            let editedSetting = this.state.settingsList.find(setting => setting[this.props.keyIndexName] === settingId);
    ...
    openAddSettingDialog = () => {
        let path = `/new${getCommonSearchParamFromURL(this.props.location.search)}`
        this.props.history.push(path)
        this.setState({
            isSettingDialogOpen: true, editedSetting: undefined, isButtonSaveEnabled: true
        })

    };

    handleChangeForm = (newEditedSetting, ignoredSettings = [], forceDisableSave = false) => {
        if (this.props.match.params.id !== 'new') {
            if (forceDisableSave) {
                this.setState({
                ...
                this.setState({
                    ...
                    isButtonSaveEnabled:
                        !isEqual(
                            omit(this.state.settingsList.find(setting => setting[this.props.keyIndexName] === newEditedSetting[this.props.keyIndexName]), ignoredSettings),
    ...
    handleCloseDialogForm = () => {
        if (this.state.isButtonSaveEnabled
            || (!this.state.isButtonSaveEnabled &&
                !isEqual(this.state.settingsList.find(setting => setting[this.props.keyIndexName] === this.state.editedSetting[this.props.keyIndexName]), this.state.editedSetting)))
        {
            ...
            let path = `${basePath}${getCommonSearchParamFromURL(this.props.location.search)}`
            this.props.history.push(path)
            this.setState({ isSettingDialogOpen: false, editedSetting: undefined })
        }
    };
    ...
    handleLeave = () => {
        ...
        let path = `${basePath}${getCommonSearchParamFromURL(this.props.location.search)}`
        this.props.history.push(path)
        this.setState({ isSettingDialogOpen: false, isSaveDialogOpen: false, editedSetting: undefined })
    }
    ...
    render() {
        return (
            <View {...this.props} {...this.state}
                ...
                action={this.props.match.params.id === 'new' ? 'Post' : 'Put'}
            />
        );
    }
}
...
SettingsUI.propTypes = {
    apiGetSettings: PropTypes.func.isRequired,
    apiPostSettings: PropTypes.func.isRequired,
    apiPutSettings: PropTypes.func.isRequired,
    apiDeleteSettings: PropTypes.func,
