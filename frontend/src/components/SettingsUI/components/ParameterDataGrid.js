...
import DateTimeFormatter from 'components/DataGrid/Formatters/ColumnFormatterDateTime';

const { Toolbar, Data: { Selectors } } = react_data_grid_addons;
...
class ParameterDataGrid extends React.Component {
    ...
    hasDateFormatting = (columnKey) => {
        return Boolean(
            this.props.gridColumns.find(
                column => column.key === columnKey && column.formatter === DateTimeFormatter))
    }
    ...
    dateCompare = (rows, sortColumn, sortDirection) => {
        ...
        return sortDirection === 'NONE' ? rows : sortDirection === 'ASC' ?
            cloneDeep(rows).sort(compare) : cloneDeep(rows).sort((a, b) => -1 * compare(a, b));
    }

    getInputRows = (_, props) => props.rows;
    getSortColumn = (state, ) => state.sortColumn;
    getSortDirection = (state, ) => state.sortDirection;
    getSortedRows = createSelector([this.getInputRows, this.getSortColumn,
    this.getSortDirection], (rows, sortColumn, sortDirection) => {
        ...
        return sortDirection === 'NONE' ? rows : orderBy(rows, [sortColumn], [sortDirection]);
    }
    );
    ...
    onRowClick = (rowIdx, row, column) => {
        if (row !== undefined) {
            return this.props.onRowClick(row[this.props.keyIndexName], column['key']);
        }
    };
